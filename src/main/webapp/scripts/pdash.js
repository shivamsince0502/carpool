...
console.log(rides);
        rides.forEach((item) => {
            if (rides.length === 0) {
                ...
                document.getElementById('show-user-card').style.display = "none"
                document.getElementById('search-rides-div').style.display = "block"
                document.getElementById("head-tb-search-ride").innerHTML = "No rides for you on your date. " + poolerName;
            }
            console.log(item);
            let tr = allRides.insertRow(-1);
            item.ride.carId = item.car.carId;
            let ride = item.ride;
            ...
            let cities = item.citiesOfRide
            ridecitiesMap.set(ride.rideId, cities)
            ...
            console.log(ridecitiesMap)
            var tableDataArray = new Array();
            tableDataArray = [ride.rideId, owner.ownerName, car.carName, car.carColor, cities[0], cities[cities.length - 1], ride.noOfSeats, ride.rideDate];
...
async function bookRide(el) {
    ...
    let ridedate = oCells[7].innerHTML;
    if (noOfSeats > 0) {
        let poolerId = sessionStorage.getItem("poolerId");
    ...
    payload.dateOfJourney = startTs
        ...
        await fetch(requrl, {
            ...
            console.log(ownerMap)
                ...
                let rideData = rideMap.get(rideId)
                let owner = ownerMap.get(rideData.ownerId)
                let allcitiesofride = ridecitiesMap.get(rideId)
                let carofride = carMap.get(rideData.carId);
    ...
    await fetch(uprideurl, {
        ...
        .then(async (uprides) => {
            for (const item of uprides) {
                ...
                let ownerMob = item.ownerMob;
                let city1 = item.start;
                let city2 = item.end;
                let rideDate = item.date;

                var tableDataArrayOfRide = new Array();
                let tr = alluprides.insertRow(-1)
                tableDataArrayOfRide = [rideId, carName, ownerName, ownerMob, city1, city2, rideDate];
...
function showToast(message, type) {
  ...
  const items = ['apple', 'banana', 'orange', 'grape', 'pineapple', 'watermelon'];
